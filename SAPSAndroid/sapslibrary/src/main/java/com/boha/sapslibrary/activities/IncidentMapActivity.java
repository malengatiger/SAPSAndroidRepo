...
public class IncidentMapActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Util.setCustomActionBar(ctx,getSupportActionBar(),
                "Incident Map","During last 60 days",ContextCompat.getDrawable(ctx,android.R.drawable.ic_dialog_map));
    ...
    private void getIncidents() {
        DataUtil.findIncidentsByRadius(getApplicationContext(),
                location.getLatitude(),
                location.getLongitude(), 25, new OKUtil.OKListener() {
                    @Override
    ...
    private void setMarkers() {
        Log.w(LOG,"++++++++++++ setMarkers: " + incidents.size());
        googleMap.clear();
        markers.clear();
        index = 0;
        for (PanicIncidentDTO station : incidents) {
            ...
            if (station.getPanicType().getName().contains("Theft")) {
                icon = ContextCompat.getDrawable(ctx,R.drawable.theft);
            }
            if (station.getPanicType().getName().contains("Rape")) {
                icon = ContextCompat.getDrawable(ctx,R.drawable.rape);
            }
            if (station.getPanicType().getName().contains("Burglary")) {
                icon = ContextCompat.getDrawable(ctx,R.drawable.crimescene);
            }
            if (station.getPanicType().getName().contains("Suspicious")) {
                icon = ContextCompat.getDrawable(ctx,R.drawable.pirates);
            }
            if (station.getPanicType().getName().contains("Reckless")) {
                icon = ContextCompat.getDrawable(ctx,R.drawable.crimescene);
            }
            if (station.getPanicType().getName().equalsIgnoreCase("Robbery")) {
                icon = ContextCompat.getDrawable(ctx,R.drawable.shooting);
            }
            if (station.getPanicType().getName().equalsIgnoreCase("Street")) {
                icon = ContextCompat.getDrawable(ctx,R.drawable.war);
            }
            if (station.getPanicType().getName().equalsIgnoreCase("Domestic")) {
                icon = ContextCompat.getDrawable(ctx,R.drawable.abduction);
            }
            ...
            Marker m = googleMap.addMarker(new MarkerOptions()
                    .title(station.getPanicIncidentID().toString())
                    .icon(desc)
                    .snippet(station.getPanicType().getName())
                    .position(pnt));
            markers.add(m);
            index++;
        }
    ...
    static final String LOG = IncidentMapActivity.class.getSimpleName();
